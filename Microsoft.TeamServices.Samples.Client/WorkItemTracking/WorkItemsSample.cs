...
namespace Microsoft.TeamServices.Samples.Client.WorkItemTracking
{
    ...
    public class WorkItemsSample : ClientSample
    {
        ...
        [ClientSampleMethod]
        public WorkItem CreateWorkItem(string title, string type, string project, WorkItem parent = null, string wikiPage = null, string storyPoint = null)
        {
            // Construct the object containing field values required for the new work item
            JsonPatchDocument patchDocument = new JsonPatchDocument();
            ...
            if (wikiPage != null)
            {
                patchDocument.Add(
                    new JsonPatchOperation()
                    {
                        ...
                        Value = "<div>See wiki, behaviour specs</div>"
                    });
                patchDocument.Add(
                    new JsonPatchOperation()
                    {
                        ...
                        {
                            rel = "ArtifactLink",
                            url = "vstfs:///Wiki/WikiPage/fa742a1f-cba5-4e3e-a211-56f965e0126f%2F190850b1-176a-4e4d-b754-a7ae918899af%2FCASA%2FESB%20(iPaas)%2FESB%20Phase%202%20(RPA%20(Drone)%20Registration)%2FIntegration%20Point%20Register%2FIP-23%20-%20Get%20RPA%20Reference%20Data",
                            attributes = new
                            {
                                name = "Wiki Page"
                            }
            ...
            patchDocument.Add(
                new JsonPatchOperation()
                {
                    Operation = Operation.Add,
                    Path = "/fields/System.IterationPath",
                    Value = @"Enterprise Service Bus\Sprint 11"
                });
        ...
        [ClientSampleMethod]
        public async Task CreateStoryDoneAPI(string project, string storyName, string storyPoints)
        {
            var parent = this.CreateWorkItem(storyName, "User Story", project, null, "/CASA/ESB (iPaas)/ESB Phase 2 (RPA (Drone) Registration)/Integration Point Register/IP-23 - Get RPA Reference Data", storyPoints);
            WorkItem newWorkItem;

            newWorkItem = this.CreateWorkItem("Created a Git Repo", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Generate & Commit the initial API Skeleton", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Define Build Definition", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Define Release Definition", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Implement the operation", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Deployed to the DEV environment", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Automated tests running against acceptance criteria in DEV environment", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Deployed to the TEST environment", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Automated tests running against acceptance criteria in TEST environment", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Define the higher environments in the release pipeline inc. variables", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Reasonable up to date as built documentation", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Release Documentation updated", "Task", project, parent);
            newWorkItem = this.CreateWorkItem("Vizzy up to date", "Task", project, parent);
        }
